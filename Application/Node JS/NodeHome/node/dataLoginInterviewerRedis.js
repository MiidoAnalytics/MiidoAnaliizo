...
var query = client.query("SELECT * FROM analiizo_limpio.splogininterviewer();",function(err,result){
	if(err){
		console.log('Error de conexión');
	}else{
	    ...
	    if(result.rows.length > 0){
			...
			clientRedis.set('udcLoginInterviewer', JSON.stringify(json), function(err, reply) 
			{
				console.log(reply);
			});

			clientRedis.get('udcLoginInterviewer', function(err, object) 
			{
				console.log(object);
			});
