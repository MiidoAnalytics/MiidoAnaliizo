...
public class PollRecordActivity extends ActionBarActivity implements View.OnClickListener,AdapterView.OnItemClickListener,SwipeRefreshLayout.OnRefreshListener{
    ...
    private void lauchActiveNetworkDialog(){
        AlertDialog dialog = com.miido.analiizoOBRAS.util.Dialog.confirm(this, "Verificar Conexión",
    ...
    private void launchConnectionRefusedDialog(){
        AlertDialog dialog = com.miido.analiizoOBRAS.util.Dialog.Alert(this, "Error de conexión",
    ...
    private ArrayList<Poll> getPollRecordsFromDB(long structureid)throws SQLiteException{
        ...
        for(int i = 0; i < cursor.getCount(); i++){
            ...
            poll.setTitle((poll.getSentDate() == null ? "Guardada" : "Enviada "+poll.getSentDate()));
            poll.setProjectName((poll.getSentDate() == null ? "Fecha :"+poll.getSavedDate() : "Guardada: "+poll.getSavedDate()));
            polls.add(poll);
            cursor.moveToNext();
        }
        return polls;
    ...
    private Poll findNoSentPoll(int pollId){
        ...
        for (Poll poll : polls){
            if(poll.getId() == pollId && poll.getSentDate() == null){
                return poll;
            }
        }
        return null;
    ...
    private void sendPollProcess(Integer... pollsIds){
        ...
        class SendPollProcess extends AsyncTask<Integer,String,Integer>{
             ...
             * o si courre un error al obtener los reportes desde la base de datos local.
             */
            @Override
                ...
                for(Integer id : ids){
                    ...
                    try {
                        socket.emit(properties.getProperty(PollsActivity.ENQUEUE_POLLS_SOCKET_SERVICE),
                                new JSONObject(poll.getContent()).toString(), new Ack() {
                            @Override
                            public void call(Object... args) {
                                if(args[0].equals("OK")){
    ...
    private void showAlertDialogTestApp(){
        AlertDialog d = com.miido.analiizoOBRAS.util.Dialog.Alert(this,"Información", "Esta acción no se puede ejecutar. versión de prueba");
        d.show();
    }
    ...
    private void showPopupMenu(View view) {
        ...
        popup.getMenu().getItem(1).setEnabled(poll.getSentDate() == null);
    ...
    private void launchAlertDialogSentPoll(final int position){
        AlertDialog d = com.miido.analiizoOBRAS.util.Dialog.Alert(this, "Información",
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(resultCode == RESULT_OK){
            if(requestCode == SAVED_POLL_REQUEST_CODE){
                getPollRecordFromDBProcess(currentPoll.getStructureId());
            }
        }
    }
