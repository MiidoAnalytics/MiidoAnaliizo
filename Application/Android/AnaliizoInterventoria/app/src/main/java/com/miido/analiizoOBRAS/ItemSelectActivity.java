...
public class ItemSelectActivity extends ActionBarActivity implements AdapterView.OnItemClickListener,AbsListView.MultiChoiceModeListener,SwipeRefreshLayout.OnRefreshListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setSubtitle("Seleccione un elemento");
    ...
    private void initAdapter(){
        ArrayList<String> tmpItems = (ArrayList<String>) items.clone();
        for (final String item : tmpItems) {
    ...
    private String getItemFromDB(int id) throws SQLiteException{
        String query = constants.GENERIC_SELECT_QUERY_WITH_CONDITIONS.replace("[FIELDS]", "*").replace("[TABLE]", this.tableName).replace("[CONDITIONS]", "id = " + id);
    ...
    public void refreshProcess(String key){
         ...
         * @see AsyncTask
         */
        class RefreshProcess extends AsyncTask<String,Integer,Integer>{
    ...
    public void getItemsFromDB(String key) throws SQLiteException{
        String query = constants.GENERIC_SELECT_QUERY_WITH_CONDITIONS.replace("[FIELDS]", "*").replace("[TABLE]", tableName);
        if(key == null || key.equals("")){
            query = query.replace("[CONDITIONS]", "1 = 1");
        }else {
            /*for (int c = 0; c < Constants.fieldsToFilter.length; c++) {
                if (Constants.fieldsToFilter[c][0].equals(tableName)) {
                    String conditions = Constants.fieldsToFilter[c][1];
                    conditions = conditions.replace(",", " LIKE '" + key + "%' OR ");
                    conditions += " LIKE '" + key + "%' LIMIT 20";
                    query = query.replace("[CONDITIONS]", conditions);
                }
            }*/
            query = query.replace("[CONDITIONS]","item LIKE '" + key + "%'");
        }
        sqlHelper.setQuery(query);
        sqlHelper.execQuery();
    }
