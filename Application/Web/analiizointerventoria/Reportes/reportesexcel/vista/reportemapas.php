...
<html>
    <head>
        ...
        <script>
            ...
            window.location = ("reportemapas.php");
            }
            else
            {
            window.location = ("reportemapas.php?intIdReport=" + t1);
            }
            }
                ...
                switch ($intIdTown) {
                    case 0:
                        if ($intIdReports == 1) {
                            ...
                            $strQuery = str_replace("WHERE (encuestacasas->'DOCUMENTINFO'->>'pollCity') = ?", '', $sqlTmp);
                        } else {
                            //echo 'otro query con Cero';
                            $strQuery = str_replace("AND (encuestacasas->'DOCUMENTINFO'->>'pollCity') = ?", '', $sqlTmp);
                        }
                        break;
                    default:
                        //echo 'mayor Cero';
                ...
                foreach ($TablaReportesxId as $item):
                    if (count($TablaQueryxidReporte) > 0):
                        foreach ($TablaQueryxidReporte as $item):
                            $this->lat_long = $this->lat_long . "['" . $item["PRIMER APELLIDO"] . "'," . $item["LATITUD"] . "," . $item["LONGITUD"] . '],';
                        endforeach;
                    else:
                    endif;
                endforeach;
        ...
        <div class="row">
            ...
            <div class="col-lg-4">                   
                ...
                <div class="panel-body">
                    <a href="informeAvances.php"><i class="fa fa-table fa-fw"></i> Informe de Avances</a>   
                </div>                    
            </div>
        </div>
    </div>
