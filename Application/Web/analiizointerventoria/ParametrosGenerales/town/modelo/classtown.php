...
class Town {
    ...
    public function guardar() {
        ...
        if ($this->intIdTowns) /* Modifica */ {
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_interventoria.spUpdateTown(?,?,?,?,?,?);');
            ...
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_interventoria.spInsertTown(?,?,?,?,?,?,?);');
    ...
    public function delete($intIdTowns) {
        ...
        $consulta = $conexion->prepare('select * from analiizo_interventoria.spDeletedTown(?,?);');
        $consulta->bindParam(1, $this->intIdTowns, PDO::PARAM_INT);
    ...
    public static function buscarPorId($intIdTowns) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_interventoria.spSelectedTownxId(?);');
    ...
    public static function searchByCod($strCodDepartament) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_interventoria.spselectedtownxcoddepto(?);');
    ...
    public static function recuperarTodos() {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_interventoria.spSelectedTown();');
        $consulta->execute();
