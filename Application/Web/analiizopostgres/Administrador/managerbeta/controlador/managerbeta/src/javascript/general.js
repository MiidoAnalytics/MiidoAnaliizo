...
var gCount = 0;
var groups = JSON.parse('{"fields_structure":[],"options":[],"forms":[],"forms_order":[],"handler_event":[],"HandlerFieldJoiner":[],"fieldsJoiner":[],"AditionalFieldsRules":[],"dynamicJoiner":[],"Document_info":{"projectId":"","structureName":"","structureDes":"","clientId":"1","structureVersion":1,"minVersionName":"2.24","currentAppVersion":"2.24","structureStatus":0}}');
...
function write(parameter) {
	...
	try {
		...
		$('#l_menu_return').bind("click", function(){
			window.location="http://52.27.125.67/analiizopostgres/Informedeavances/informeAvances/controlador/informeAvances.php";
		});
	} catch(error) {}
	try {
		$("#r_groups").unbind();
...
function createFieldExistentContainer(strTmp, index) {
	...
	$(itTmp).bind("change", function(){
		var dom = $(this);
		var position = dom.context.parentNode.parentNode.id;
		groups.fields_structure[position].Name = dom.context.value.toLowerCase().repare();
		...
		case 'rg':
			if(groups.options[indquestion].Field[0] == idPre){
				if(groups.options[indquestion].Options.length < 2){
					...
					iDivTmpSP.appendChild(document.createTextNode("Agregar opción"));
...
// Crear las opciones 
	...
	$(inputOption).unbind();
	$(inputOption).bind("change", function(){
	    ...
	    $("#td_context_delete").bind("click", function(){
	    	...
	    	if(groups.fields_structure[idTmp].Handler == 0){
	    		...
	    		for(idx in groups.forms){
	    			if(groups.fields_structure[idTmp].Handler == groups.forms[idx].Parent){
