...
class Departament {
    ...
    public function guardar() {
        ...
        if ($this->intIdDepartament) /* Modifica */ {
            try {
                ...
                $consulta = $conexion->prepare('select * from administracion.spUpdateDepartment(?,?,?,?,?,?);');
            ...
            try {
                ...
                $consulta = $conexion->prepare('select * from administracion.spInsertDepartment(?,?,?,?,?,?,?);');
                ...
                $procedimiento = 'select * from administracion.spInsertDepartment(' . $this->strCodDepartament . $this->strNombreDepartament . $strRegisteredUser . $dtCreateDate . ')';
    ...
    public function eliminar($intIdDepartament) {
        ...
        $consulta = $conexion->prepare('select * from administracion.spDeletedDepartment(?,?);');
    ...
    public static function buscarPorId($intIdDepartament) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from administracion.spSelectedDepartmentxId(?);');
    ...
    public static function buscarPorCod($strCodDepartament) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from administracion.spSelectedDepartamentxCod(?);');
    ...
    public static function recuperarTodos() {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from administracion.spSelectedDepartment();');
        $consulta->execute();
