...
class Tablet {
    ...
    public function insert() {
        ...
        if ($this->intIdTablet) /* Modifica */ {
            try {
                ...
                $consulta = $conexion->prepare('select * from administracion.spUpdateTablets(?,?,?,?,?);');
            ...
            try {
                ...
                $consulta = $conexion->prepare('select * from administracion.spInsertTablets(?,?,?,?,?,?,?);');
                ...
                /* $procedimiento = 'select * from administracion.spInsertDepartment('. $this->strNomtablets. $this->strKeytablets . $strRegisteredUser . $dtFechaRegistro . ')';
    ...
    public function delete($intIdTablet) {
        ...
        $consulta = $conexion->prepare('select * from administracion.spDeletedTablets(?,?);');
        $consulta->bindParam(1, $this->intIdTablet, PDO::PARAM_INT);
     ...
     public function delelteRelTabletsInter($intIdTablet) {
        ...
        $conexion->exec("set names utf8");
        $dtModifiedDate = date("Ymd");
        $consulta = $conexion->prepare('select * from administracion.delelteRelTabletsInter(?,?);');
        $consulta->bindParam(1, $this->intIdTablet, PDO::PARAM_INT);
        $consulta->bindParam(2, $dtModifiedDate, PDO::PARAM_STR);
        $consulta->execute();
        $conexion = null;
    ...
    public static function searchById($intIdTablet) {
        ...
        $consulta = $conexion->prepare('select * from administracion.spSelectedTabletxId(?);');
    ...
    public static function getAllTablets() {
        ...
        $consulta = $conexion->prepare('select * from administracion.spSelectedTablets();');
        $consulta->execute();
    ...
    public static function getAllTabletsFree() {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from administracion.spSelectedTabletsFree();');
        $consulta->execute();
        $registros = $consulta->fetchAll();
        $conexion = null;
        return $registros;
