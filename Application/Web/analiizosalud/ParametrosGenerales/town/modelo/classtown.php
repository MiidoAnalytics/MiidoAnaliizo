...
class Town {
    ...
    public function guardar() {
        ...
        if ($this->intIdTowns) /* Modifica */ {
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spUpdateTown(?,?,?,?,?,?);');
            ...
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spInsertTown(?,?,?,?,?,?,?);');
    ...
    public function delete($intIdTowns) {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spDeletedTown(?,?);');
        $consulta->bindParam(1, $this->intIdTowns, PDO::PARAM_INT);
    ...
    public static function buscarPorId($intIdTowns) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedTownxId(?);');
    ...
    public static function searchByCod($strCodDepartament) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spselectedtownxcoddepto(?);');
    ...
    public static function recuperarTodos() {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedTown();');
        $consulta->execute();
