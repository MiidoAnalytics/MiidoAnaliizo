...
class Departament {
    ...
    public function guardar() {
        ...
        if ($this->intIdDepartament) /* Modifica */ {
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spUpdateDepartment(?,?,?,?,?,?);');
            ...
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spInsertDepartment(?,?,?,?,?,?,?);');
                ...
                $procedimiento = 'select * from analiizo_salud.spInsertDepartment(' . $this->strCodDepartament . $this->strNombreDepartament . $strRegisteredUser . $dtCreateDate . ')';
    ...
    public function eliminar($intIdDepartament) {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spDeletedDepartment(?,?);');
    ...
    public static function buscarPorId($intIdDepartament) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedDepartmentxId(?);');
    ...
    public static function buscarPorCod($strCodDepartament) {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedDepartamentxCod(?);');
    ...
    public static function recuperarTodos() {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedDepartment();');
        $consulta->execute();
