...
class Tablet {
    ...
    public function insert() {
        ...
        if ($this->intIdTablet) /* Modifica */ {
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spUpdateTablets(?,?,?,?,?);');
            ...
            try {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spInsertTablets(?,?,?,?,?,?,?);');
                ...
                /* $procedimiento = 'select * from analiizo_salud.spInsertDepartment('. $this->strNomtablets. $this->strKeytablets . $strRegisteredUser . $dtFechaRegistro . ')';
    ...
    public function delete($intIdTablet) {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spDeletedTablets(?,?);');
        $consulta->bindParam(1, $this->intIdTablet, PDO::PARAM_INT);
     ...
     public function delelteRelTabletsInter($intIdTablet) {
        ...
        $conexion->exec("set names utf8");
        $dtModifiedDate = date("Ymd");
        $consulta = $conexion->prepare('select * from analiizo_salud.delelteRelTabletsInter(?,?);');
        $consulta->bindParam(1, $this->intIdTablet, PDO::PARAM_INT);
        $consulta->bindParam(2, $dtModifiedDate, PDO::PARAM_STR);
        $consulta->execute();
        $conexion = null;
    ...
    public static function searchById($intIdTablet) {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedTabletxId(?);');
    ...
    public static function getAllTablets() {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedTablets();');
        $consulta->execute();
    ...
    public static function getAllTabletsFree() {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedTabletsFree();');
        $consulta->execute();
        $registros = $consulta->fetchAll();
        $conexion = null;
        return $registros;
