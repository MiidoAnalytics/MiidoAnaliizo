...
if ($flag == 1) {
    ...
    if ($encuesta && $proyecto) {
        ...
        $TablaInformeEncuestadoresPorMunicipio = InformeAvances::recuperarInformeEncuestadoresPorMunicipio($proyecto, $encuesta);
        $TablaEncuestasEncuestadores = InformeAvances::recuperarInformeEncuestasEncuestador($proyecto, $encuesta);
    } else {
        $idusuario = $_SESSION['userid'];
        $proyectos = InformeAvances::ObtenerProyectosUsuerId($idusuario);
    }
