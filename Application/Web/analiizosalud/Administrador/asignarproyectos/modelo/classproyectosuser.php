...
class ProyectosUser {
    ...
    public function guardar($intidproyecto, $idusuarios) {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spUInsertProyectoUser(?,?,?,?,?,?);');
    ...
    public static function getAllProyectosUser() {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");
        $consulta = $conexion->prepare('select * from analiizo_salud.spselectedproyectosUser();');
        $consulta->execute();
        $registros = $consulta->fetchAll();
        $conexion = null;
        return $registros;
    ...
    public static function validarUserProyecto($intidproyecto, $idusuarios) {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedcountuserProyecto(?,?);');
        $consulta->bindParam(1, $intidproyecto, PDO::PARAM_INT);
        $consulta->bindParam(2, $idusuarios, PDO::PARAM_INT);
        $consulta->execute();
