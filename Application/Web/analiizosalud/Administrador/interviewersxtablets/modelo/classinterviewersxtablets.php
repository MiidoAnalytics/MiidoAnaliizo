...
class Interviewersxtablets 
{
    ...
    public function insert() 
    {
        ...
        /*if ($this->intIdInterviewers && $this->intIdTablet)  Modifica 
        {
            ...
            {            
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spUpdateCountry(?,?,?,?,?,?);');
        ...
        //{
            try 
            {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spInsertInterviewersxtablets(?,?,?,?,?);');
                ...
                /*$procedimiento = 'select * from analiizo_salud.spInsertCountry('. $this->intIdTablet . $this->strNombreidPais . $strRegisteredUser . $dtCreateDate . ')';
    ...
    public function updateTablexInterviewer($intIdInterviewers, $intIdTablet) 
    {
        ...
        $conexion->exec("set names utf8");
                 ...
                 try 
            {
                ...
                $consulta = $conexion->prepare('select * from analiizo_salud.spUpdateInterviewersxtablets(?,?,?,?,?);');
                $consulta->bindParam(1, $this->intIdInterviewers, PDO::PARAM_INT);            
                $consulta->bindParam(2, $this->intIdTablet, PDO::PARAM_INT);
                $consulta->bindParam(3, $strRegisteredUser, PDO::PARAM_STR);
                $consulta->bindParam(4, $IstatusId, PDO::PARAM_STR);
    ...
    public function freeTablets($intIdInterviewers, $intIdTablet)
    {
        ...
        $strRegisteredUser = $_SESSION['user'];
        $dtCreateDate = date("Ymd");
        $consulta = $conexion->prepare('select * from analiizo_salud.spFreeTabletsInterviewer(?,?);');
    ...
    public function freeTablets2($intIdInterviewers, $intIdTablet)
    {
        ...
        $strRegisteredUser = $_SESSION['user'];
        $dtCreateDate = date("Ymd");
        $consulta = $conexion->prepare('select * from analiizo_salud.spfreetabletsinterviewer2(?,?);');
        $consulta->bindParam(1, $intIdInterviewers, PDO::PARAM_INT);
        $consulta->bindParam(2, $intIdTablet, PDO::PARAM_INT);
        $consulta->execute();
        $conexion = null;
    ...
    public static function buscarPorId($intIdInterviewers) 
    {
        $conexion = new ConexionAnaliizoPostgres();
        $conexion->exec("set names utf8");       
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedCountryxId(?);');
    ...
    public static function searchUserWithTablet($intIdInterviewers) 
    {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedUserWithTablexId(?);');
        $consulta->bindParam(1, $intIdInterviewers, PDO::PARAM_STR);
        $consulta->execute();
        $registro = $consulta->fetch();
        $conexion = null;
    ...
    public static function getAll() 
    {
        ...
        $consulta = $conexion->prepare('select * from analiizo_salud.spSelectedInterviewersxtablets();');        
        $consulta->execute();
